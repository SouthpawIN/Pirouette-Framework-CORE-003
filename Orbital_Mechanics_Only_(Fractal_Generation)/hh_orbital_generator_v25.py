...
def run_ablation(eng, stiff, hel, mass, tok, n_tokens, out_path, prompt_ids=[],
    ...
    arms = {
        'baseline': dict(use_flow=False, use_lambda=False),
        'L_only':   dict(use_flow=False, use_lambda=True),
        'F_only':   dict(use_flow=True,  use_lambda=False, flow_field=flow_field),
        'FL_both':  dict(use_flow=True,  use_lambda=True,  flow_field=flow_field),
    }
    results = {}
    for name, kw in arms.items():
        res = run_v24(eng, stiff, hel, mass, tok, prompt_ids, n_tokens,
                      verbose=False, rng=np.random.default_rng(seed), **kw)
        results[name] = res
        _print_arm(name, res)

    # Flow phase-shuffle null arm (F_only with shuffled flow field)
    if flow_field_shuf is not None:
        res_shuf = run_v24(eng, stiff, hel, mass, tok, prompt_ids, n_tokens,
                           verbose=False, rng=np.random.default_rng(seed),
                           use_flow=True, use_lambda=False, flow_field=flow_field_shuf)
        results['F_shuffled'] = res_shuf
        _print_arm('F_shuffled (flow null)', res_shuf)

    base = results['baseline']; L = results['L_only']; F = results['F_only']; FL = results['FL_both']

    # ── Evaluate pre-registered hypotheses ────────────────────────────────────
    h1 = 'PASS' if (F['content_frac'] > base['content_frac'] + 0.05
                    and F['noise_frac'] < 0.10) else 'NULL'
    h2 = 'PASS' if (L['ttr'] > base['ttr'] * 1.20
                    and L['noise_frac'] < 0.10) else 'NULL'

    # H-V25-003 attribution
    if h2 == 'PASS' and h1 == 'NULL':
        attribution = 'temperature'   # FALSIFIED: geometry not load-bearing
    elif h1 == 'PASS' and h2 == 'NULL':
        attribution = 'geometry'
    elif h1 == 'PASS' and h2 == 'PASS':
        better_fl = (FL['content_frac'] >= max(F['content_frac'], base['content_frac'])
                     and FL['ttr'] >= max(L['ttr'], base['ttr']))
        attribution = 'both' if better_fl else 'mixed'
    else:
        attribution = 'neither'       # nothing moved — both knobs inert

    h_phase = 'n/a'
    if 'F_shuffled' in results:
        h_phase = 'PASS' if F['content_frac'] > results['F_shuffled']['content_frac'] * 1.10 else 'NULL'

    summary = {
        'H_V25_001_flow_content':  {'status': h1,
            'F_content': F['content_frac'], 'base_content': base['content_frac']},
        'H_V25_002_lambda_ttr':    {'status': h2,
            'L_ttr': L['ttr'], 'base_ttr': base['ttr']},
        'H_V25_003_attribution':   {'attribution': attribution,
            'note': 'temperature => geometry NOT load-bearing (falsified); '
                    'geometry/both => successor thesis supported'},
        'H_V25_PHASE_flow_null':   {'status': h_phase,
            'F_content': F['content_frac'],
            'F_shuf_content': results.get('F_shuffled',{}).get('content_frac')},
    }
    print(f"\n  {'='*58}")
    print(f"  H-V25-001 (flow->content):  {h1}", flush=True)
    print(f"  H-V25-002 (lambda->ttr):    {h2}", flush=True)
    print(f"  H-V25-003 ATTRIBUTION:      {attribution.upper()}", flush=True)
    print(f"  H-V25-PHASE (flow null):    {h_phase}", flush=True)
...
def main():
    p = argparse.ArgumentParser(description='Orbital Generator v25 — ML-100')
    p.add_argument('command', choices=['generate','ablation','fish','phase_null'])
    ...
    p.add_argument('--corpus',        default=None,
                   help='text file to build the flow field (KNOB 1). Needs --model.')
    ...
    p.add_argument('--output',        default='v25_out.json')
    args = p.parse_args()

    print(f"\n{'='*60}", flush=True)
    print(f"  Orbital Generator v25 — ML-100  [{args.command}]", flush=True)
    print(f"{'='*60}\n", flush=True)
    ...
    # ── Build flow field (KNOB 1) if a corpus is provided ────────────────────
    flow_field = None; flow_field_shuf = None
    if args.corpus:
        if tok is None:
            print("  [warn] --corpus needs --model (tokenizer); flow disabled.", flush=True)
        else:
