...
# ══════════════════════════════════════════════════════════════════════════════
# v27 — ML-102 : THE SECOND EYE  (LocalFrame)
# ══════════════════════════════════════════════════════════════════════════════
#
# ENGINEERING MODE — NOT a certification. Physicality constraint OFF. We are
# instantiating flow to make the addressing space generate text, as a
# proof-of-concept. We add information, but NO LLM weights and NO corpus: the
# added information is the engram READ AT SECOND ORDER.
#
# THE IDEA (Keaton's "second eye / fractal 2 at the same address"):
#   Fractal 1  -> token addresses (j1, ksi): tells you WHERE you are.
#   Fractal 2  -> the LOCAL FRAME of the engram at that address: the orientation
#                 of the neighbor cloud in (j1, ksi, spin) space. Tells you which
#                 way the neighborhood LEANS.
#   Flow = the lean. Parallax between the two readings of the same point gives a
#   direction without any sequence data — it is the engram's own second-order
#   structure, so it is corpus-agnostic by construction.
#
#   coherence = 1 - circular_dispersion of the neighbor lean. High coherence = a
#   well-defined frame (confident push). Low = isotropic neighborhood (let lambda
#   explore). This coherence also feeds the lambda controller's "structured" term.
#
# HONEST RISK (told to the user): if the local frame is dominated by the same
# density gradient that made Zone A a function-word swamp, the second eye just
# re-points into the swamp. The tell is whether TTR/diversity survives. We keep
# the lambda controller wired in alongside, because that is the demonstrated,
# corpus-independent win — the demo should lean on what worked.
# ══════════════════════════════════════════════════════════════════════════════

FRAME_K        = 40      # neighbors used to estimate the local frame
FRAME_WEIGHT   = 1.0     # blend weight of frame axis into cone axis (scaled by coherence)
FRAME_RADIUS   = 35.0    # max j1 distance (deg) for a neighbor to count


class LocalFrame:
    """
    The second eye. Reads the engram's neighbor-cloud orientation at any address.
    Engram-only: uses (j1, ksi, spin) already present. No corpus, no LM weights.

    Precomputes a coarse grid of frame axes so per-step query is O(1).
    """
    def __init__(self, eng, nj=FLOW_NJ, nk=FLOW_NK, k=FRAME_K, radius=FRAME_RADIUS):
        self.nj = nj; self.nk = nk
        self.j1v = eng['j1_v']; self.ksv = eng['ksi_v']
        self.spin = eng.get('spin_angle', None)
        self.axis = np.zeros((nj, nk))     # frame lean as a j1-direction (deg)
        self.coh  = np.zeros((nj, nk))     # coherence in [0,1]
        self.built = False
        self._build(k, radius)

    def _build(self, k, radius):
        j1v = self.j1v; ksv = self.ksv; spin = self.spin
        for cj in range(self.nj):
            j1_c = (cj + 0.5) * (360.0 / self.nj)
            for ck in range(self.nk):
                ksi_c = (ck + 0.5) / self.nk
                # gather neighbors near this cell center
                dj = np.abs(((j1v - j1_c + 180) % 360) - 180)
                near = np.where(dj < radius)[0]
                if len(near) < 5:
                    continue
                # weight by proximity in (j1, ksi)
                w = np.exp(-0.5 * (dj[near] / radius) ** 2
                           - 0.5 * ((ksv[near] - ksi_c) / 0.15) ** 2)
                if w.sum() < 1e-6:
                    continue
                # The "lean": weighted circular mean of neighbor j1 positions
                # RELATIVE to the cell center = which way the cloud is offset.
                rel = np.radians(((j1v[near] - j1_c + 180) % 360) - 180)
                s = np.sum(w * np.sin(rel)); c = np.sum(w * np.cos(rel))
                lean = math.degrees(math.atan2(s, c))
                # coherence = resultant length (0=isotropic, 1=tight lean)
                R = math.hypot(s, c) / w.sum()
                # fold spin orientation in if present: align lean toward the
                # neighborhood's mean spin axis (second-order structure)
                if spin is not None:
                    ss = np.sum(w * np.sin(spin[near])); cc = np.sum(w * np.cos(spin[near]))
                    spin_axis = math.degrees(math.atan2(ss, cc))
                    spin_R = math.hypot(ss, cc) / w.sum()
                    # blend lean with spin axis, weighted by their coherences
                    bs = R * math.sin(math.radians(lean)) + spin_R * math.sin(math.radians(spin_axis))
                    bc = R * math.cos(math.radians(lean)) + spin_R * math.cos(math.radians(spin_axis))
                    lean = math.degrees(math.atan2(bs, bc))
                    R = 0.5 * (R + spin_R)
                self.axis[cj, ck] = lean
                self.coh[cj, ck]  = R
        self.built = True
    ...
    def query(self, j1, ksi):
        """Returns (frame_axis_offset_deg, coherence). axis is a *relative* lean
        to be added to the current j1 to form an absolute steering direction."""
        if not self.built:
            return 0.0, 0.0
        cj, ck = self._cell(j1, ksi)
        return float(self.axis[cj, ck]), float(self.coh[cj, ck])

    def coverage(self):
        return float((self.coh > 0.05).mean())


def build_local_frame(eng, shuffled=False, seed=0):
    """
    Build the second eye. shuffled=True permutes the spin assignments across
    tokens (keeps marginal spin distribution, destroys which token sits where),
    so Frame_shuffled tests whether the neighbor-cloud orientation is real
    structure or an artifact of the marginal density.
    """
    if not shuffled:
        return LocalFrame(eng)
    rng = np.random.default_rng(seed)
    eng2 = dict(eng)
    if eng.get('spin_angle') is not None:
        eng2['spin_angle'] = rng.permutation(eng['spin_angle'])
    eng2['ksi_v'] = rng.permutation(eng['ksi_v'])
    return LocalFrame(eng2)


# ══════════════════════════════════════════════════════════════════════════════
# v28 — ML-103 : ARRIVAL-CONDITIONED SECOND-ORDER FRAME  (DirectionalFrame)
# ══════════════════════════════════════════════════════════════════════════════
#
# The grammar gap. v27's LocalFrame reads a single lean per cell — position only.
# Proximity on the manifold is NOT adjacency in text, so position-only steering
# gets the right REGION (semantics) but not the right SUCCESSOR (syntax).
#
# DirectionalFrame is the legitimate pure-geometry second-order object: the frame
# at a cell depends on the ARRIVAL DIRECTION. We bucket each cell's neighbors by
# the angular sector they occupy relative to the cell center, and store a separate
# lean per sector. At runtime the generator's actual arrival direction
# (j1_h - j1_prev) selects the bucket: "approaching from sector X, the structure
# ahead bends toward Y." This is the manifold analog of momentum-dependent exit —
# the closest thing to grammatical context that the engram's own geometry can
# carry, with NO corpus and NO LM.
#
# HONEST BOUNDARY: grammar may be irreducibly sequential and simply not encoded in
# spatial geometry. If arrival-conditioning produces local agreement, that's a
# real finding. If it does not, we have cleanly located the boundary: semantics
# and exploration are geometric; syntax is not. Either outcome is a true claim.
#
# NULL: the shuffled DirectionalFrame (permuted spin/ksi) tells us whether the
# arrival->lean dependence is real structure or an artifact of marginal density.
# ══════════════════════════════════════════════════════════════════════════════

DFRAME_SECTORS = 8       # arrival-direction buckets (45 deg each)
DFRAME_RADIUS  = 35.0
DFRAME_MIN_N   = 4       # min neighbors in a sector to trust its lean
DFRAME_WEIGHT  = 1.0


class DirectionalFrame:
    """
    Arrival-conditioned frame. Per cell, per arrival-sector: the lean of the
    neighbor structure on the FAR side (the side you're heading toward).
    Engram-only. query(j1, ksi, arrival_dir) -> (lean_offset_deg, coherence).
    """
    def __init__(self, eng, nj=FLOW_NJ, nk=FLOW_NK, sectors=DFRAME_SECTORS,
                 radius=DFRAME_RADIUS):
        self.nj = nj; self.nk = nk; self.S = sectors
        self.j1v = eng['j1_v']; self.ksv = eng['ksi_v']
        self.spin = eng.get('spin_angle', None)
        # axis[cj,ck,s] = lean (deg) given arrival in sector s; coh likewise
        self.axis = np.zeros((nj, nk, sectors))
        self.coh  = np.zeros((nj, nk, sectors))
        self.built = False
        self._build(radius)

    def _sector(self, ang_deg):
        # map an angle (deg, any range) to a sector index in [0, S)
        return int(((float(ang_deg) % 360) / 360.0) * self.S) % self.S
    ...
    def _build(self, radius):
        ...
        for cj in range(self.nj):
            ...
            for ck in range(self.nk):
                ksi_c = (ck + 0.5) / self.nk
                dj_signed = ((j1v - j1_c + 180) % 360) - 180   # signed offset to center
                dj = np.abs(dj_signed)
                near = np.where(dj < radius)[0]
                if len(near) < 5:
                    continue
                # bearing of each neighbor relative to the cell, in the (j1,ksi) plane
                bearing = np.degrees(np.arctan2((ksv[near] - ksi_c) * 360.0, dj_signed[near]))
                for s in range(self.S):
                    sec_center = (s + 0.5) * (360.0 / self.S)
                    # neighbors whose bearing falls in this arrival sector's FAR side:
                    # we want the structure you'd head INTO when arriving from sector s,
                    # i.e. neighbors on the opposite bearing (sec_center + 180).
                    far = (sec_center + 180.0) % 360.0
                    bdiff = np.abs(((bearing - far + 180) % 360) - 180)
                    sel = near[bdiff < (360.0 / self.S)]
                    if len(sel) < DFRAME_MIN_N:
                        continue
                    dj_sel = np.abs(((j1v[sel] - j1_c + 180) % 360) - 180)
                    w = np.exp(-0.5 * (dj_sel / radius) ** 2)
                    wsum = w.sum() or 1e-9
                    rel = np.radians(((j1v[sel] - j1_c + 180) % 360) - 180)
                    ss = np.sum(w*np.sin(rel)); cc = np.sum(w*np.cos(rel))
                    lean = math.degrees(math.atan2(ss, cc))
                    R = math.hypot(ss, cc) / wsum
                    if spin is not None:
                        sp_s = np.sum(w*np.sin(spin[sel])); sp_c = np.sum(w*np.cos(spin[sel]))
                        spin_axis = math.degrees(math.atan2(sp_s, sp_c))
                        spin_R = math.hypot(sp_s, sp_c) / wsum
                        bs = R*math.sin(math.radians(lean)) + spin_R*math.sin(math.radians(spin_axis))
                        bc = R*math.cos(math.radians(lean)) + spin_R*math.cos(math.radians(spin_axis))
                        lean = math.degrees(math.atan2(bs, bc)); R = 0.5*(R+spin_R)
                    self.axis[cj, ck, s] = lean
                    self.coh[cj, ck, s]  = R
        self.built = True
    ...
    def query(self, j1, ksi, arrival_dir):
        """arrival_dir = signed j1 step that brought us here (deg). Returns
        (lean_offset_deg, coherence) for that arrival sector."""
        if not self.built:
            return 0.0, 0.0
        cj, ck = self._cell(j1, ksi)
        s = self._sector(arrival_dir)
        return float(self.axis[cj, ck, s]), float(self.coh[cj, ck, s])

    def coverage(self):
        return float((self.coh > 0.05).any(axis=2).mean())


def build_directional_frame(eng, shuffled=False, seed=0):
    if not shuffled:
        return DirectionalFrame(eng)
    rng = np.random.default_rng(seed)
    eng2 = dict(eng)
    if eng.get('spin_angle') is not None:
        eng2['spin_angle'] = rng.permutation(eng['spin_angle'])
    eng2['ksi_v'] = rng.permutation(eng['ksi_v'])
    return DirectionalFrame(eng2)
...
def run_v24(eng, stiff_lut, helic_lut, mass, tokenizer,
            ...
            use_frame=False, local_frame=None,
            use_dframe=False, dframe=None,
            rng=None,
            verbose=True):
    """
    ...
    for step in range(n_tokens):
        ...
        # ── KNOB 4: SECOND EYE (LocalFrame) — engram-only instantiated flow ──
        frame_coh = 0.0
        if use_frame and local_frame is not None and local_frame.built:
            frame_off, frame_coh = local_frame.query(j1_h, ksi_h)
            if frame_coh > 0.:
                frame_axis = (j1_h + frame_off) % 360.
                w_fr = FRAME_WEIGHT * frame_coh
                s = (1. - w_fr)*math.sin(math.radians(cone_axis)) + w_fr*math.sin(math.radians(frame_axis))
                c = (1. - w_fr)*math.cos(math.radians(cone_axis)) + w_fr*math.cos(math.radians(frame_axis))
                cone_axis = math.degrees(math.atan2(s, c)) % 360.

        # ── KNOB 5: ARRIVAL-CONDITIONED FRAME (DirectionalFrame) — grammar try ──
        dframe_coh = 0.0
        if use_dframe and dframe is not None and dframe.built:
            arrival = ang_diff(j1_prev, j1_h)   # signed step that brought us here
            dframe_off, dframe_coh = dframe.query(j1_h, ksi_h, arrival)
            if dframe_coh > 0.:
                dframe_axis = (j1_h + dframe_off) % 360.
                w_df = DFRAME_WEIGHT * dframe_coh
                s = (1. - w_df)*math.sin(math.radians(cone_axis)) + w_df*math.sin(math.radians(dframe_axis))
                c = (1. - w_df)*math.cos(math.radians(cone_axis)) + w_df*math.cos(math.radians(dframe_axis))
                cone_axis = math.degrees(math.atan2(s, c)) % 360.
        ...
        if use_lambda:
            ...
            rough = max(rough_geom, flow_conf, frame_coh, dframe_coh)   # confident flow/frame => commit
    ...
    return {'mode': mode, 'text': text, 'tokens': tokens_out, 'zone_counts': zone_c,
            ...
            'use_helicity': use_helicity, 'use_frame': use_frame,
            'use_dframe': use_dframe,
...
def run_ablation(eng, stiff, hel, mass, tok, n_tokens, out_path, prompt_ids=[],
                 ...
                 local_frame=None, local_frame_shuf=None,
                 dframe=None, dframe_shuf=None):
    """
    v27 ablation — ML-102.  (extends v26 with the second-eye LocalFrame arms)

    Arms: baseline / L_only / F_only / H_only / HL_both
          + Frame_only / Frame_L  (the v27 second eye, engram-only flow)
          + F_shuffled / H_shuffled / Frame_shuffled (nulls)

    v27 is ENGINEERING (proof-of-concept), not certification. The Frame_shuffled
    null still runs so you can SEE whether the second eye carries real structure or
    just re-points into the density swamp: Frame_only content/ttr vs Frame_shuffled.
    """
    def _run(**kw):
    ...
    if local_frame is not None:
        arms['Frame_only'] = dict(use_frame=True, local_frame=local_frame)
        arms['Frame_L']    = dict(use_frame=True, local_frame=local_frame, use_lambda=True)
    if dframe is not None:
        arms['DFrame_only'] = dict(use_dframe=True, dframe=dframe)
        arms['DFrame_L']    = dict(use_dframe=True, dframe=dframe, use_lambda=True)
        if local_frame is not None:
            arms['Frame+DFrame_L'] = dict(use_frame=True, local_frame=local_frame,
                                          use_dframe=True, dframe=dframe, use_lambda=True)
    ...
    if local_frame_shuf is not None:
        results['Frame_shuffled'] = _run(use_frame=True, local_frame=local_frame_shuf)
        _print_arm('Frame_shuffled (second-eye null)', results['Frame_shuffled'])
    if dframe_shuf is not None:
        results['DFrame_shuffled'] = _run(use_dframe=True, dframe=dframe_shuf)
        _print_arm('DFrame_shuffled (arrival-frame null)', results['DFrame_shuffled'])
...
def main():
    p = argparse.ArgumentParser(description='Orbital Generator v28 — ML-103')
    ...
    # v27 additions — the second eye (engram-only, NO corpus needed)
    p.add_argument('--use_frame',     action='store_true',
                   help='enable KNOB 4 (LocalFrame second eye) in generate')
    p.add_argument('--use_dframe',    action='store_true',
                   help='enable KNOB 5 (arrival-conditioned DirectionalFrame) in generate')
    p.add_argument('--seed',          type=int, default=0)
    p.add_argument('--output',        default='v27_out.json')
    args = p.parse_args()

    print(f"\n{'='*60}", flush=True)
    print(f"  Orbital Generator v27 — ML-102  [{args.command}]", flush=True)
    print(f"{'='*60}\n", flush=True)
    ...
    # ── Second eye (LocalFrame): engram-only, built always — NO corpus needed ──
    print(f"  Building second eye (LocalFrame) from engram...", flush=True)
    local_frame      = build_local_frame(eng, shuffled=False)
    local_frame_shuf = build_local_frame(eng, shuffled=True, seed=args.seed)
    print(f"  Second-eye coverage: {local_frame.coverage():.1%} of cells have a frame", flush=True)
    # ── Arrival-conditioned frame (DirectionalFrame): the grammar attempt ──
    print(f"  Building arrival-conditioned frame (DirectionalFrame)...", flush=True)
    dframe      = build_directional_frame(eng, shuffled=False)
    dframe_shuf = build_directional_frame(eng, shuffled=True, seed=args.seed)
    print(f"  DirectionalFrame coverage: {dframe.coverage():.1%} of cells have >=1 sector lean", flush=True)
    ...
    if args.corpus:
        if tok is None:
            ...
            print(f"  Helicity field built  kappa={args.kappa}", flush=True)
    ...
    if args.command == 'generate':
        res = run_v24(eng, stiff, hel, mass, tok, prompt_ids, args.n_tokens,
                      ...
                      use_frame=args.use_frame, local_frame=local_frame,
                      use_dframe=args.use_dframe, dframe=dframe,
        ...
        run_ablation(eng, stiff, hel, mass, tok, args.n_tokens, args.output, prompt_ids,
                     ...
                     local_frame=local_frame, local_frame_shuf=local_frame_shuf,
                     dframe=dframe, dframe_shuf=dframe_shuf)
